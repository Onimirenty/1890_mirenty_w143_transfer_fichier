...
public class Serveur
{
    ...
    protected void sendBytes2(byte[] dataBytes,ServerSocket serveursocket) throws Exception
    {
        ...
        OutputStream outStream=socket.getOutputStream();
            ...
            System.out.println("array of length "+dataBytes.length+" transfered");
    }
    ...
    public byte[] FichierEnbyte(String pathString) throws Exception
    {
        ...
        Path path=Paths.get(pathString);//meme que pathstring mais de type path
    ...
    public byte[][] splitByteArray(byte[] input) {
        ...
        System.out.println(input[tab1.length]);
        System.out.println("taille de depart ="+input.length);
        ...
        /*
            ...
            tab1,length(indice a partir du quel on commence a copier le tab d origine)
            tab2(table destinataire de la copy)
            0(indice de depart ou commencer a copier le tableau d origine)
            tab2.length(nombre d element du tableau a copier)
        */
    ...
    public String AddStringBetweenNameAndExt(String name,String thing)
    {
        String nameWOext=name.substring(0,name.lastIndexOf("."));
        String extWdot=name.substring(nameWOext.length(),name.length());
        return nameWOext+thing+extWdot;
    ...
    public String createDirectory_to_save_into() throws Exception
    {
        ...
        File nvrep=new File(parent,"AAA");
        ...
        else if(nvrep.exists() && nvrep.isDirectory())
        {   
            System.out.println("le repertoir existe deja ");
            rep=nvrep.getAbsolutePath();
        }
        else
        {
    ...
    public  String detect_path_to_save_into()
    {
        ...
        if(os.indexOf("win")>=0)
        {
            ...
            for (File file : roots) 
            {
                if(file.toString().equalsIgnoreCase("E:"+File.separator))
                {
                    path="E:"+File.separator;
                    return  path;        
                }
                else if(file.toString().equalsIgnoreCase("D:"+File.separator))
                {
                    path="D:"+File.separator;
                    return  path;
                }
                else if(file.toString().equalsIgnoreCase("c:"+File.separator))
                {
